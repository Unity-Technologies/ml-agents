...
def make_demo_buffer(
    ...
    for idx, current_pair_info in enumerate(pair_infos):
        ...
        demo_raw_buffer[AgentBufferKey.ENVIRONMENT_REWARDS].append(next_reward)
        ...
        if (
            ...
            if behavior_spec.action_spec.continuous_size > 0:
                demo_raw_buffer[AgentBufferKey.CONTINUOUS_ACTION].append(
                    current_pair_info.action_info.vector_actions_deprecated
                )
            else:
                demo_raw_buffer[AgentBufferKey.DISCRETE_ACTION].append(
                    current_pair_info.action_info.vector_actions_deprecated
                )
        else:
            if behavior_spec.action_spec.continuous_size > 0:
        ...
        demo_raw_buffer[AgentBufferKey.PREV_ACTION].append(previous_action)
        if next_done:
