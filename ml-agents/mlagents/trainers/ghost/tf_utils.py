...
class TensorFlowVariables(object):
    ...
    def __init__(self, output, sess=None, input_variables=None):
        ...
        variable_list = [
            v for v in tf.global_variables() if v.op.node_def.name in variable_names
        ]
        ...
        for k, var in self.variables.items():
            self.placeholders[k] = tf.placeholder(
                var.value().dtype, var.get_shape().as_list(), name="Placeholder_" + k
            )
    ...
    def _check_sess(self):
        ...
        assert self.sess is not None, (
            "The session is not set. Set the "
    ...
    def set_weights(self, new_weights):
        ...
        self._check_sess()
        assign_list = [
            self.assignment_nodes[name]
            for name in new_weights.keys()
            if name in self.assignment_nodes
        ]
        assert assign_list, (
            "No variables in the input matched those in the "
