...
class PreTraining(object):
    ...
    def __init__(self, sess, policy_model: LearningModel, brain, parameters):
        ...
        if (
            self.use_recurrent
            and not self.brain.vector_action_space_type == "continuous"
        ):
        ...
        self.out_dict = {"loss": self.loss, "update": self.update_batch}
        if parameters["normalize"]:
            self.out_dict[
                "update_normalization"
            ] = self.policy_model.update_normalization
    ...
    def _add_loss(self, learning_rate):
        ...
        if self.brain.vector_action_space_type == "continuous":
            self.expert_action = tf.placeholder(
                shape=[None, action_size[0]], dtype=tf.float32, name="teacher_action"
            )
            ...
            self.expert_action = tf.placeholder(
                shape=[None, len(action_size)], dtype=tf.int32
            )
            expert_action_oh = tf.concat(
                [
                    tf.one_hot(self.expert_action[:, i], action_size[i])
            ...
            self.loss = -tf.reduce_sum(
                tf.log(tf.nn.softmax(log_probs) + 0 * 1e-10) * expert_action_oh
    ...
    def _update(self, mini_batch, num_sequences):
        ...
        if self.brain.vector_observation_space_size > 0:
            ...
            feed_dict[self.policy_model.vector_in] = mini_batch["vector_obs"].reshape(
                [-1, apparent_obs_size]
            )
        ...
        if self.use_recurrent:
            feed_dict[self.policy_model.memory_in] = np.zeros(
                [num_sequences, self.m_size]
            )
            if not self.brain.vector_action_space_type == "continuous":
                ...
                feed_dict[self.policy_model.prev_action] = np.zeros(
                    (
