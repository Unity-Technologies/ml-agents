...
class ActionModel(nn.Module):
    ...
    def _get_probs_and_entropy(
    ...
    ) -> Tuple[ActionLogProbs, Optional[torch.Tensor], Optional[torch.Tensor]]:
        """
        ...
        :params dists: The DistInstances tuple
        :return: An ActionLogProbs tuple, continuous entropy, and discrete entropy.
        """
    ...
    def forward(
    ...
    ) -> Tuple[AgentAction, ActionLogProbs, Optional[torch.Tensor], Optional[torch.Tensor]]:
        """
        ...
        ActionLogProbs, continuous entropy, and discrete entropy.
        """
        dists = self._get_dists(inputs, masks)
        actions = self._sample_action(dists)
        log_probs, continuous_entropy, discrete_entropy = self._get_probs_and_entropy(actions, dists)
        return (actions, log_probs, continuous_entropy, discrete_entropy)
