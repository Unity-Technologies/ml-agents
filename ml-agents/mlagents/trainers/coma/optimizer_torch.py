...
class TorchCOMAOptimizer(TorchOptimizer):
    class COMAValueNetwork(torch.nn.Module, Critic):
        """
        The COMAValueNetwork uses the MultiAgentNetworkBody to compute the value
        and COMA baseline for a variable number of agents in a group that all
        ...
        def baseline(
            ...
            """
            The COMA baseline marginalizes the action of the agent associated with self_obs.
    ...
    def __init__(self, policy: TorchPolicy, trainer_settings: TrainerSettings):
        ...
        :param policy: A TorchPolicy object that will be updated by this COMA Optimizer.
        ...
        self._critic_baseline = TorchCOMAOptimizer.COMAValueNetwork(
            reward_signal_names,
            policy.behavior_spec.observation_specs,
            network_settings=trainer_settings.network_settings,
            action_spec=policy.behavior_spec.action_spec,
    ...
    def create_reward_signals(
        ...
        for reward_signal in reward_signal_configs.keys():
            if reward_signal != RewardSignalType.EXTRINSIC:
                logger.warning(
                    f"Reward signal {reward_signal.value.capitalize()} is not supported with the COMA trainer; "
                    "results may be unexpected."
                )
        super().create_reward_signals(reward_signal_configs)
        # Make sure we add the groupmate rewards in COMA, so agents learn how to help each
