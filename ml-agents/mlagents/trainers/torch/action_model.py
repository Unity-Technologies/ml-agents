...
class ActionModel(nn.Module):
    ...
    def _get_dists(
        self, inputs: torch.Tensor, mode_oh, masks: torch.Tensor
    ) -> DistInstances:
        """
        ...
        if self._continuous_distribution is not None:
            continuous_dist = self._continuous_distribution(inputs, mode_oh)
    ...
    def _get_probs_and_entropy(
        ...
        return action_log_probs, entropies, stats
    ...
    def evaluate(
        ...
        log_probs, entropies, stats = self._get_probs_and_entropy(actions, dists)
    ...
    def forward(
        ...
        # Use the sum of entropy across actions, not the mean
        entropy_sum = torch.sum(entropies, dim=1)
        return (actions, log_probs, entropy_sum, stats)
