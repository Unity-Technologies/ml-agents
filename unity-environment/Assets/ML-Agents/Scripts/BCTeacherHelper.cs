...
namespace MLAgents
{
    ...
    public class BCTeacherHelper : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            Monitor.Log("Recording experiences " + recordKey.ToString(),
                recordExperiences.ToString());
            float timeSinceBufferReset = Time.time - bufferResetTime;
            Monitor.Log("Seconds since buffer reset " + resetKey.ToString(),
                Mathf.FloorToInt(timeSinceBufferReset));
        }
