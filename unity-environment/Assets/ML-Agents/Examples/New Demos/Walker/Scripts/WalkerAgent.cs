...
public class WalkerAgent : Agent
{
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        ...
        foreach (var item in bodyParts)
        {
            var velSm = item.Value.rb.angularVelocity.sqrMagnitude;
            velocityPenalty += velSm / 10f;

            if (item.Value.joint)
            {
                var currentTorqueSm = item.Value.joint.currentTorque.sqrMagnitude;
                torquePenalty += currentTorqueSm / 10000f;
            }
        }

        torquePenaltyFinal = 0.0001f * torquePenalty;
        velocityPenaltyFinal = 0.0001f * velocityPenalty;
