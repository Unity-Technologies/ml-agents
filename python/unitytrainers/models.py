...
class LearningModel(object):
    ...
    def create_dc_actor_critic(self, h_size, num_layers):
        ...
        self.action_masks = tf.placeholder(shape=[None, sum(self.act_size)], dtype=tf.float32,
                                           name="action_masks")
        output = self.create_discrete_action_masking_layer(policy_branches, self.action_masks,
                                                           self.act_size)
        ...
        self.action_holder = tf.placeholder(shape=[None, len(policy_branches)], dtype=tf.int32,
                                            name="action_holder")
        self.selected_actions = tf.concat([
            tf.one_hot(self.action_holder[:, i], self.act_size[i]) for i in
            range(len(self.act_size))], axis=1)

        self.all_old_log_probs = tf.placeholder(shape=[None, sum(self.act_size)], dtype=tf.float32,
                                                name='old_probabilities')
