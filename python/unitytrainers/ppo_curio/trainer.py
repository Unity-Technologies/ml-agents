...
from unitytrainers.ppo_curio import PPOCurioModel
...
class PPOCurioTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed):
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            self.model = PPOCurioModel(env.brains[brain_name],
            ...
            if stored_info is None:
                ...
                if not stored_info.local_done[idx]:
                    ...
                    if self.use_states:
                        ...
                        self.training_buffer[agent_id]['next_states'].append(next_info.vector_observations[next_idx])
                    if self.use_recurrent:
                    ...
                    self.training_buffer[agent_id]['rewards'].append(next_info.rewards[next_idx] + intrinsic_rewards[next_idx])
    ...
    def update_model(self):
        ...
        total_v, total_p, total_f, total_i = 0, 0, 0, 0
            ...
            for l in range(len(self.training_buffer.update_buffer['actions']) // n_sequences):
                ...
                if self.use_states:
                    if self.brain.vector_observation_space_type == "continuous":
                        ...
                        feed_dict[self.model.next_state] = np.array(
                            _buffer['next_states'][start:end]).reshape(
                            [-1, self.brain.vector_observation_space_size * self.brain.num_stacked_vector_observations])
                    else:
                        feed_dict[self.model.vector_in] = np.array(
                ...
                v_loss, p_loss, f_loss, i_loss, _ = self.sess.run(
                    [self.model.value_loss, self.model.policy_loss, self.model.forward_loss, self.model.inverse_loss,
        ...
        self.stats['forward_loss'].append(total_f)
        self.stats['inverse_loss'].append(total_i)
