...
for %%i in (%SRC_DIR%\*.proto) do (
    protoc --proto_path=proto --csharp_opt=internal_access --csharp_out=%DST_DIR_C% %%i
...
protoc --proto_path=proto --csharp_out %DST_DIR_C% --grpc_out=internal_access:%DST_DIR_C% %SRC_DIR%\%GRPC% --plugin=protoc-gen-grpc=grpc_csharp_plugin.exe --csharp_opt=internal_access
