...
namespace MLAgents
{
    ...
    public struct AgentInfo
    {
        ...
        /// <summary>
        /// Remove the visual observations from memory. Call at each time step
    ...
    public abstract class Agent : MonoBehaviour
    {
        ...
        void OnEnableHelper(Academy academy)
        {
            ...
            else
            {
                Debug.Log(
                    ...
                    $"GameObject {gameObject.name} was initialized without a brain.");
            }
        ...
        void SendInfoToBrain()
        {
            ...
            {
                throw new UnityAgentsException("Vector Observation size mismatch between continuous " +
                    $"agent {gameObject.name} and brain {brain.name}. " +
                    $"Was Expecting {brain.brainParameters.vectorObservationSize} but received {m_Info.vectorObservation.Count}. ");
            }
            ...
            {
                throw new UnityAgentsException(
                    $"Not enough cameras/renderTextures for agent {gameObject.name} : Brain {brain.name} expecting at " +
                    $"least {brain.brainParameters.cameraResolutions.Length} cameras/renderTextures but only {visualObservationCount} were present.");
            }
        ...
        /// <summary>
        /// Updates the value of the action.
        /// </summary>
        /// <param name="value">Updates the action value.</param>
        ...
        public static Texture2D ObservationToTexture(Camera obsCamera, int width, int height)
        {
            ...
            const RenderTextureReadWrite readWrite = RenderTextureReadWrite.Default;
        ...
        /// <summary>
        /// Converts a RenderTexture and corresponding resolution to a 2D texture.
        /// </summary>
        /// <returns>The 2D texture.</returns>
        ...
        public static Texture2D ObservationToTexture(RenderTexture obsTexture, int width, int height)
        {
            ...
            {
                throw new UnityAgentsException(
                    $"RenderTexture {obsTexture.name} : width/height is {obsTexture.width}/{obsTexture.height} " +
                    "brain is expecting {width}/{height}.");
            }
