...
to your model's latest checkpoint. (Note: There is a known bug on Windows that
causes the saving of the model to fail when you early terminate the training,
it's recommended to wait until Step has reached the max_steps parameter you
set)You can now embed this trained model into your Learning Brain by following
the steps below:
