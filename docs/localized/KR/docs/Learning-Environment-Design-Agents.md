...
`Policy` 정책 클래스는 에이전트 자체의 의사 결정 논리(Logic)을 추상화합니다. 따라서 다양한 에이전트에 동일한 정책을 사용할 수 있습니다. 정책이 결정을 결정하는 것은 에이전트와 연결된 `Behavior Parameters`에 의존합니다.
...
실험으로서, 3DBall 에이전트가 속도를 관측하지 못하게 할 수 있습니다.
...
최고의 학습을 위해, 특정 벡터 구간을 [-1, +1] 또는 [0, 1]로 정규화 하는 것이 좋습니다. 값을 정규화 할 때, PPO 뉴럴 네트워크는 보상의 수렴 속도를 증가시킬 수 있습니다. 하지만 이러한 값으로 정규화를 하는 것은 필수적이지는 않지만, 뉴럴 네트워크를 사용할 때 고려할만 합니다. 관측 컨포넌트의 범위 변동이 클수록, 학습에 영향을 미칠 가능성이 높아집니다.
