...
the Agents connected to Brains present in the `Training Hub` are doing. 
...
for the Brains in the `Training Hub` with the `Train` checkbox checked. If there
are Brains in the `Training Hub` with the 
`Train` checkbox checked, simply call `step()` with no arguments.
